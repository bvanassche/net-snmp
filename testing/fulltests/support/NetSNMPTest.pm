...
sub require_feature {
    ...
    for my $file ("$updir/include/net-snmp/net-snmp-config.h",
                  "$updir/include/net-snmp/agent/agent_module_config.h",
                  "$updir/include/net-snmp/agent/mib_module_config.h") {
        my $fh = new IO::File($file);
        next if !$fh;
        while (<$fh>) {
            if (/#define $feature\b/) {
